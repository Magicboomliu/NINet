...
from utils.plane_loss import GetDisparityPlanePixelLoss
    ...
    def forward(self, disp_infer,surface_normal_infer, disp_gt):
        ...
        if (type(surface_normal_infer) is tuple) or (type(surface_normal_infer) is list):
            for i, input_ in enumerate(surface_normal_infer):
                ...
                # Directly Normal Loss
                smooth_l1_loss+=self.smoothl1(pred_normal,target_normal)*self.weights[i]

                # Plane_loss
                plane_loss+=GetDisparityPlanePixelLoss(disp_infer[i],pred_normal,supervised_gt_disp_,
                                                    target_normal) * self.weights[i]
                ...
                supervised_gt_disp = supervised_gt_disp_[mask]
                disp_smooth_l1_loss+=self.smoothl1(disp_infer_,supervised_gt_disp)*self.weights[i]
            ...
            # Plane Loss
            plane_loss+=GetDisparityPlanePixelLoss(disp_infer,pred_normal,disp_gt,
                                                    target_normal)
        ...
        if disp_smooth_l1_loss!=0:
            ...
            ratio_v2 = disp_smooth_l1_loss_value*1.0/(plane_loss.item()+1e-10)
            
            total_loss = smooth_l1_loss*ratio +disp_smooth_l1_loss*1.2 + plane_loss *ratio_v2/2
        else:
            total_loss = smooth_l1_loss*192 + disp_smooth_l1_loss + plane_loss
