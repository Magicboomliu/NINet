...
class MultiScaleLossN(nn.Module): # here make sure scales_list[1] is not 0
    ...
    def forward(self,disp_infer,disp_gt,normal_infer,img_left,img_right):
            ...
            for i, input_ in enumerate(disp_infer):
                if not self.apply_rec:
                    if i==0:
                        ...
                        target_ = F.interpolate(disp_gt,scale_factor=1./pow(2,i),mode='bilinear',align_corners=False)/pow(2,i)
                    
                    traget_normal = get_normal(target_)
                    ...
                    if self.mask:
                        ...
                        mask = (target_ < 192/pow(2,i)) & (target_ > 0)
                        mask.detach_()
                        input_ = input_[mask]
        ...
        ratio = disp_loss.item()/(normal_loss.item()+1e-8)
        total_loss = disp_loss * 1.0 + normal_loss * 1.0*ratio
