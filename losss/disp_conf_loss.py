...
def get_confidence_loss(disp_infer,infer_disp_conf,disp_gt):
    ...
    loss1 = loss1 * confidence_cond_lowwer
    
    #c
    loss2 = infer_disp_conf
    loss2 = loss2 * confidence_cond_upper

    total_loss = loss1 + loss2
    total_loss = torch.sum(total_loss) /(torch.sum(confidence_cond_lowwer)+torch.sum(confidence_cond_upper))
...
class MultiScaleLoss(nn.Module):
    ...
    def forward(self, disp_infer, disp_gt, disp_conf_infer,img_left, img_right):
        ...
        if (type(disp_infer) is tuple) or (type(disp_infer) is list):
            for i, input_ in enumerate(disp_infer):
                if not self.apply_rec:
                    ...
                    if self.mask:
                        ...
                        conf_loss = get_confidence_loss(input_*mask,disp_conf_infer[i],target_*mask)
                        mask.detach_()
                        input_ = input_[mask]
                        target_ = target_[mask]
            ...
            confidence_loss = get_confidence_loss(disp_infer*mask,disp_conf_infer,disp_gt*mask)
