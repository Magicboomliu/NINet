...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'gt_slant' in sample.keys():
            slant = np.transpose(sample['gt_slant'],(2,0,1))
            sample['gt_slant'] = torch.from_numpy(slant)
        ...
        if self.img_height > ori_height or self.img_width > ori_width:
            ...
            if 'gt_normal' in sample.keys():
                sample['gt_normal'] = np.lib.pad(sample['gt_normal'],
                                         ...
                                         constant_values=0)
            if 'gt_slant' in sample.keys():
                sample['gt_slant'] = np.lib.pad(sample['gt_slant'],
                                         ((top_pad, 0), (0, right_pad), (0, 0)),
                                         mode='constant',
                                         constant_values=0)
            ...
            if 'gt_slant' in sample.keys():
                sample['gt_slant'] = self.crop_img(sample['gt_slant'])
            if 'pseudo_disp' in sample.keys():
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            if 'gt_slant' in sample.keys():
                sample['gt_slant'] = np.copy(np.flipud(sample['gt_slant']))
