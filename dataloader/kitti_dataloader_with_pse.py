...
class StereoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode=='test' or self.mode=='val':
            ...
            sample['img_left'] = read_img(sample_path['left'])  # [H, W, 3]
            sample['img_right'] = read_img(sample_path['right'])
            img_left = transform.resize(sample['img_left'], [384,1248], preserve_range=True)
            img_right = transform.resize(sample['img_right'], [384,1248], preserve_range=True)
            ...
            # Get GT Disparity and pseudo Disparity
            # Keep the Original Size
            H,W = sample['pseudo_disp'].shape
            
            if H!=374:
                gt_disp = read_kitti_step1(sample_path['disp'])
                gt_disp = gt_disp.crop((W-1242, H-375, W, H))
            ...
            if H == 374:
                pseudo_disp_scale[1:,4:] = sample['pseudo_disp']
