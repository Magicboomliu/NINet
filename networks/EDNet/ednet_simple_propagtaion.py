...
class EDNet(nn.Module):
    def __init__(self, batchNorm=False, max_disp=192, input_channel=3, res_type='normal',
        ...
        for _ in range(4):
            self.affinity_propagation_list.append(Affinity_propagation())
        ...
        if self.res_type == 'attention':
            ...
            residual = res_submodule_feature_porpagation
        else:
            raise NotImplementedError("Wrong residual type")
        
        self.res_submodule_2 = residual(scale=2, input_layer=64, out_planes=32,propagation_node=self.propagation_list[0])
        self.res_submodule_1 = residual(scale=1, input_layer=32, out_planes=32,propagation_node=self.propagation_list[1])
        self.res_submodule_0 = residual(scale=0, input_layer=32, out_planes=32,propagation_node=self.propagation_list[2])
    ...
    def forward(self, img_left, img_right,gt_disp,training=False):
        ...
        cur_gt_disp = F.interpolate(gt_disp,size=[pr3.size(-2),pr3.size(-1)],mode='bilinear',align_corners=False)/8
        cur_gt_normal = get_normal(cur_gt_disp)
        pr3 = self.affinity_propagation_list[0](cur_gt_normal,img_left,img_right,pr3)
        ...
        pr2 = self.affinity_propagation_list[1](cur_gt_normal,img_left,img_right,pr2)
        ...
        pr1 = self.affinity_propagation_list[2](cur_gt_normal,img_left,img_right,pr1)
        ...
        cur_gt_disp = gt_disp
        cur_gt_normal = get_normal(cur_gt_disp)
        pr0 = self.affinity_propagation_list[3](cur_gt_normal,img_left,img_right,pr0)
