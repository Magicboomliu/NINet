...
class Modal_Learning(nn.Module):
    def __init__(self, batchNorm=False, max_disp=192, 
                input_channel=3, res_type='normal',feature_exchange=False):
        super(Modal_Learning, self).__init__()
        
        self.max_disp = max_disp
        self.res_type = res_type
        self.feature_exchange = feature_exchange # Whether to change the feature at 1/8 normal-disparity prediction
        ...
        if self.feature_exchange == False:
        ...
        elif self.res_type =='deform':
            disp_residual = res_submodule_deform_only
    ...
    def forward(self,img_left,img_right,is_training= False):
        ...
        # Normal at 1/8 Scale
        if self.feature_exchange:
            cur_feature_n = torch.cat((iconv3_d,iconv3_n),dim=1)
        else:
            cur_feature_n = iconv3_n
        
        normal3 = self.normal3(cur_feature_n)
        normal3 = F.normalize(normal3,dim=1)

        # Disp at 1/8 Scale
        if self.feature_exchange:
            cur_feature_d = torch.cat((iconv3_d,iconv3_n),dim=1)
        else :
            cur_feature_d = iconv3_d

        disp3 = self.disp3(cur_feature_d)
...
if __name__ =="__main__":
    ...
    liuzihuanet = Modal_Learning(res_type='deform_norm',feature_exchange=True).cuda()
