...
from networks.EDNet.pretrain_affinity.Residual_Types import res_submodule_lstm, res_submodule_with_normal_deform_Simple


class cross_layer_fusion(nn.Module):
    def __init__(self,input_dim,feature_dim,scale=0):
        super(cross_layer_fusion,self).__init__()
        self.input_dim = input_dim
        self.feature_dim = feature_dim
        self.scale = scale
        self.mask = nn.Sequential(
            nn.Conv2d(input_dim,1,1,1),
            nn.Sigmoid())
        self.conv1 = nn.Sequential(
            nn.Conv2d(input_dim+feature_dim,feature_dim,3,1,1),
    ...
    def forward(self,h,iconv):
        if h.size(-2)!=iconv.size(-2):
            h = F.interpolate(h,size = [iconv.size(-2),iconv.size(-1)],mode='bilinear',align_corners=False)
        
        h_mask = self.mask(h)
        h_new = h * h_mask
        concated = torch.cat((h_new,iconv),dim=1)
        
        iconv = self.conv1(concated)
...
class EDNet(nn.Module):
    def __init__(self, batchNorm=False, max_disp=192, input_channel=3, res_type='normal',
        ...
        # Use what kind of the residual model: Decoder Part
        if self.res_type == 'attention':
            residual = res_submodule_attention
        elif self.res_type =='deform_norm':
            residual = res_submodule_with_normal_deform_Simple
        elif self.res_type == 'deform_lstm':
            residual = res_submodule_lstm
        else:
            raise NotImplementedError("Wrong residual type")
        if self.res_type == 'deform_lstm':
            
            self.res_submodule_2 = residual(scale=2,input_layer=64,out_planes=32,iteration=2,use_lstm=True)
            self.fusion1 = cross_layer_fusion(32,32,0)
            self.res_submodule_1 = residual(scale=1,input_layer=32,out_planes=32,iteration=2,use_lstm=True)
            self.fusion2 = cross_layer_fusion(32,32,0)
            self.res_submodule_0 = residual(scale=0,input_layer=32,out_planes=32,iteration=2,use_lstm=True)
    ...
    def forward(self, img_left, img_right,assisted_normal,training=False):
        ...
        if self.res_type == 'deform_lstm':
            pr2,h2,c2 = self.res_submodule_2(img_left,img_right,pr2,cur_normal,iconv2,h=None,c=None)

        else:
            pr2 = self.res_submodule_2(img_left, img_right, pr2,cur_normal,iconv2)
        ...
        if self.res_type == 'deform_lstm':
            iconv1,h2_new = self.fusion1(h2,iconv1)
        
            pr1,h1,c1 = self.res_submodule_1(img_left,img_right,pr1,cur_normal,iconv1,h=h2_new,c=None)
       
        else:
            pr1 = self.res_submodule_1(img_left, img_right, pr1,cur_normal,iconv1) 
        ...
        if self.res_type == 'deform_lstm':
            iconv0,h1_new = self.fusion2(h1,iconv0)
            
            pr0,h0,c0 = self.res_submodule_0(img_left,img_right,pr0,cur_normal,iconv0,h=h1_new,c=None)
            
        else:
            pr0 = self.res_submodule_0(img_left, img_right, pr0,cur_normal,iconv0)
...
if __name__=="__main__":
    ...
    mynet = EDNet(res_type='deform_lstm').cuda()
    disp_pyramid = mynet(input_tensor,input_tensor,assisted_normal,True)
