...
from networks.utilsNet.nl_propagation_edition import NonLocalSpatialPropagation,Args
...
class EDNet(nn.Module):
    def __init__(self, batchNorm=False, max_disp=192, input_channel=3, res_type='normal',
        ...
        self.args = Args(prop_times=1,affinity='TGASS',nlk=3,affinity_gamma=0.5,conf_prop=True,fusion=True)
        
        self.affinity_propagation_list=nn.ModuleList()
        for _ in range(4):
            self.affinity_propagation_list.append(NonLocalSpatialPropagation(self.args,guidance_channel=12))
    ...
    def forward(self, img_left, img_right,assisted_normal,training=False):
        ...
        pr3 = self.affinity_propagation_list[0](pr3,cur_normal,img_left,img_right,confidence3)
        ...
        pr2 = self.affinity_propagation_list[1](pr2,cur_normal,img_left,img_right,confidence2)
        ...
        pr1 = self.affinity_propagation_list[2](pr1,cur_normal,img_left,img_right,confidence1)
        ...
        if not training:
            pr0 = self.affinity_propagation_list[3](pr0,cur_normal,img_left,img_right,confidence0,True)
        else:
            pr0 = self.affinity_propagation_list[3](pr0,cur_normal,img_left,img_right,confidence0)
...
if __name__=="__main__":
    left_image = torch.randn(1,3,384,1280).cuda()
    right_image = torch.randn(1,3,384,1280).cuda()
    assisted_normal = torch.randn(1,3,384,1280).cuda()

    mynet = EDNet(res_type='deform_norm',squeezed_volume=True)
    mynet = torch.nn.DataParallel(mynet, device_ids=[0]).cuda()

    disparity_pyramid,conf_pyramid = mynet(left_image,right_image,assisted_normal,False)
