...
from networks.utilsNet.Normal_Estimation import NormalEstimation
...
class EDNet(nn.Module):
    def __init__(self, batchNorm=False, max_disp=192, input_channel=3, res_type='normal',
                 squeezed_volume=False,norm_block=4):
        ...
        self.norm_block = norm_block # Number of Normal Estimation Module
        ...
        # for idx in range(self.norm_block):
        #     self.normal_estimation_list.append(NormalEstimation(is_first=idx,use_deform=True))
    ...
    def forward(self, img_left, img_right,gt_disp,training=False):
        ...
        # Normal Estimation at 1/8 Scale
        #normal_1_8,normal_feature3 = self.normal_estimation_list[0](in_conv3b,pr3,False,img_left)
        normal_1_8 = get_normal(pr3/8)
        ...
        #normal_1_4, normal_feature2 = self.normal_estimation_list[1](normal_feature3,pr2,False,img_left)
        ...
        normal_1_4 = get_normal(pr2/4)
        res2 = self.res_submodule_2(img_left, img_right, pr2,normal_1_4,iconv2)
        ...
        #normal_1_2, normal_feature1 = self.normal_estimation_list[2](normal_feature2,pr1,False,img_left)
        normal_1_2 = get_normal(pr1/2)
        res1 = self.res_submodule_1(img_left, img_right, pr1,normal_1_2,iconv1) #
        pr1 = pr1 + res1
        pr1 = self.relu1(pr1)
        ...
        #normal_0_0, normal_feature0 = self.normal_estimation_list[3](normal_feature1,pr0,False,img_left)        
        # cur_gt_disp = gt_disp
        # cur_gt_normal = get_normal(cur_gt_disp)
        normal_0_0 = get_normal(pr0)
        res0 = self.res_submodule_0(img_left, img_right, pr0,normal_0_0,iconv0)
        pr0 = pr0 + res0
        pr0 = self.relu0(pr0)

        if training:
            return [pr0, pr1, pr2, pr3],[normal_0_0,normal_1_2,normal_1_4,normal_1_8]
        else: 
            return pr0,normal_0_0
...
if __name__=="__main__":
    ...
    disp_pyramid,normal_pyramid = mynet(input_tensor,input_tensor,gt_disp,True)
    print_tensor_shape(disp_pyramid)
    print("----------------------------")
    print_tensor_shape(normal_pyramid)