...
''' When the hidden Nodes is biger than One'''
...
class Affinity_propagation_with_conf(nn.Module):
    def __init__(self, kernel_size=3, stride=1):
        ...
        self.affinity_matrix_conv = compute_affinity_matrix(input_channel=12,feature_channel=32)
        self.softmax = nn.Softmax(dim=1)
    def forward(self, normal_estimation,left_image,right_image,coarse_disparity,confidence=None):
        """
        params:
            normal_estimation: Estimated normal tensor with dim [N, 3, H, W] for the 
        ...
        input_features = torch.cat((normal_estimation,cur_left_img,cur_right_img,warped_error),dim=1)
...
if __name__=="__main__":
    ...
    normal_estimation = torch.randn(2,3,100,100)
    affinity_propagation = Affinity_propagation_with_conf(kernel_size=3,stride=1)
    # refine disparity
    refined_disparity = affinity_propagation(normal_estimation,left_image,right_image,corase_disp,confidenceMap)
