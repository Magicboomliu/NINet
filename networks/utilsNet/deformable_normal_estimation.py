...
import sys
sys.path.append("../../")
from networks.utilsNet.resblocks import DeconBlock,DeformBottleneck
...
class NormalEstimation(nn.Module):
    def __init__(self,is_first=0,use_deform=False):
        ...
        if self.is_first==0:
            self.feature_conv1 = conv2d(24,16)
        if self.is_first==1:
...
# Normal residual Estimation
class NormalEstimationResidual(nn.Module):
    def __init__(self,is_first=0,use_deform=False):
        super(NormalEstimation,self).__init__()
        self.is_first= is_first
        self.use_deform = use_deform
    ...
    def forward(self,features,disp,residual=False,left_image=None):
        ...
        if self.is_first==0:
            ...
            features = self.feature_conv3(features)
            cur_left_image = F.interpolate(left_image,scale_factor=1./pow(2,3-self.is_first),mode='bilinear',
            align_corners=False)
            left_image_feature = self.left_image_conv(cur_left_image)
...
if __name__=="__main__":
    features = torch.randn(1,24,36,72).cuda()
