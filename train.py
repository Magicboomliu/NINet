...
from train_file.kitti_pretrainer import DisparityTrainer
from losss.disp_conf_loss_kitti import multiscaleloss
...
def main(opt):
    ...
    trainer = DisparityTrainer(opt.lr, opt.devices, opt.dataset, opt.trainlist, opt.vallist, opt.datapath, opt.batch_size, opt.maxdisp,opt.use_deform,opt.pretrain,opt.model, test_batch=opt.test_batch,normal_path=opt.norm_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--norm_path",type=str,help='Normal Path Saved Path ',default='models_saved/Ednet_normal/norm0016.pth')
